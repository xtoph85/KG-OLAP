...
filtered <- cases[cases$case == 'SesameHTTPRepo-ExecuteDeltaQuery-Evaluate' & (cases$analysis == 'at.jku.dke.kgolap.demo.analyses.DemoAnalysis3DMergeUnion' | cases$analysis == 'at.jku.dke.kgolap.demo.analyses.DemoAnalysis4DMergeUnion'), ]

grouped <- filtered %>% group_by(case, dataset, dimensions, statementsAssertedInferred, contexts, statementsInsertedDeleted)
...
summarized <- summarized[!is.na(summarized$medianWallTimeDuration),]

pdf("performance-merge.pdf", width=15, height=5) 

par(mfrow=c(1,3), oma=c(1.5,3.5,2.5,0), mar=c(5,5,2,2), mgp=c(4,2,0))

plot(y = summarized$medianWallTimeDuration, x = summarized$statementsAssertedInferred/1000000, pch = c(10,11,6,20)[summarized$dimensions], xlab="Repository size [million statements]", ylab="", ylim=c(0,1600), yaxs="i", xlim=c(0,37), xaxs="i", cex = 2, cex.axis = 2, cex.lab=2)
legend(2.5, 1500, legend=c("3D", "4D"), pch=c(6, 20), cex = 2)

#plot(y = summarized$medianWallTimeDuration, x = summarized$contexts, pch = c(10,11,1,4)[summarized$dimensions], xlab="Number of contexts", ylab="", ylim=c(0,1600), yaxs="i", xlim=c(0,4100), xaxs="i")
#legend(250, 1450, legend=c("3D", "4D"), pch=c(6, 4))

plot(y = summarized$medianWallTimeDuration, x = summarized$statementsAssertedInferred/1000000, pch = c(1,4,5)[as.factor(summarized$contexts)], xlab="Repository size [million statements]", ylab="", ylim=c(0,1600), yaxs="i", xlim=c(0,37), xaxs="i", cex = 2, cex.axis = 2, cex.lab=2)
legend(2.5, 1500, legend=c("1365 Contexts", "2501 Contexts", "3906 Contexts"), pch=c(1, 4, 5), cex = 2)

plot(y = summarized$medianWallTimeDuration, x = summarized$statementsInsertedDeleted/1000000, pch = c(1,4,5)[as.factor(summarized$contexts)], xlab="Delta size [million statements]", ylab="", ylim=c(0,1600), yaxs="i", xlim=c(0,74), xaxs="i", cex = 2, cex.axis = 2, cex.lab=2)
legend(5, 1500, legend=c("1365 Contexts", "2501 Contexts", "3906 Contexts"), pch=c(1, 4, 5), cex = 2)

mtext(text="Median run time [s]", side=2, line=0, outer=TRUE, cex = 1.2)

mtext("(b) Merge", outer = TRUE, cex = 2)
