...
filtered <- cases[cases$case == 'SesameHTTPRepo-ExecuteDeltaQuery-Evaluate' & (cases$analysis == 'at.jku.dke.kgolap.demo.analyses.DemoAnalysisBaselineAbstractionIndividualGenerating' | cases$analysis == 'at.jku.dke.kgolap.demo.analyses.DemoAnalysis3DAbstractionIndividualGenerating' | cases$analysis == 'at.jku.dke.kgolap.demo.analyses.DemoAnalysis4DAbstractionIndividualGenerating'), ]
...
pdf("performance-abstraction-individualgenerating.pdf", width=15, height=5) 

par(mfrow=c(1,3), oma=c(1.5,3.5,2.5,0), mar=c(5,5,2,2), mgp=c(4,2,0))

plot(y = summarized$medianWallTimeDuration, x = summarized$statementsAssertedInferred/1000000, pch = c(10,0,6,20)[summarized$dimensions], xlab="Repository size [million statements]", ylab="", ylim=c(0,1150), yaxs="i", xlim=c(0,37), xaxs="i", cex = 2, cex.axis = 2, cex.lab=2)
legend(2, 1100, legend=c("3D", "4D", "Baseline"), pch=c(6, 20, 0), cex = 2)

plot(y = summarized$medianWallTimeDuration, x = summarized$statementsAssertedInferred/1000000, pch = c(0,1,4,5)[as.factor(summarized$contexts)], xlab="Repository size [million statements]", ylab="", ylim=c(0,1150), yaxs="i", xlim=c(0,37), xaxs="i", cex = 2, cex.axis = 2, cex.lab=2)
legend(2, 1100, legend=c("1365 Contexts", "2501 Contexts", "3906 Contexts", "Baseline"), pch=c(1, 4, 5, 0), cex = 2)

plot(y = summarized$medianWallTimeDuration, x = summarized$statementsInsertedDeleted/1000000, pch = c(0,1,4,5)[as.factor(summarized$contexts)], xlab="Delta size [million statements]", ylab="", ylim=c(0,1150), yaxs="i", xlim=c(0,9), xaxs="i", cex = 2, cex.axis = 2, cex.lab=2)
legend(0.4, 1100, legend=c("1365 Contexts", "2501 Contexts", "3906 Contexts", "Baseline"), pch=c(1, 4, 5, 0), cex = 2)
...
mtext("(b) Individual-Generating Abstraction", outer = TRUE, cex = 2)
